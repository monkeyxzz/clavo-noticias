...
// .wrangler/tmp/pages-yU3zu2/functionsWorker-0.7141437322715465.mjs
...
async function onRequest(context) {
  ...
  items = items.filter((it) => it.url && !seen.has(it.url) && seen.add(it.url)).sort((a, b) => (b.ts || 0) - (a.ts || 0));
  const perSource = {};
  items = items.filter((it) => {
    perSource[it.source] = (perSource[it.source] || 0) + 1;
    return perSource[it.source] <= PER_SOURCE;
  }).slice(0, MAX_ITEMS).map(({ ts, ...rest }) => rest);
  const body = JSON.stringify({ items, updated: (/* @__PURE__ */ new Date()).toISOString() });
...
function extractImage(block) {
  let m = block.match(/<media:content[^>]+url=["']([^"']+)["']/i) || block.match(/<media:thumbnail[^>]+url=["']([^"']+)["']/i) || block.match(/<enclosure[^>]+url=["']([^"']+)["'][^>]*type=["']image/i);
...
__name2(extractImage, "extractImage");
function decodeEntities(str) {
  if (!str) return "";
  return str.replace(/&#x([0-9a-f]+);/gi, (_, h) => String.fromCodePoint(parseInt(h, 16))).replace(/&#(\d+);/g, (_, n) => String.fromCodePoint(parseInt(n, 10))).replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;|&apos;/g, "'").replace(/&nbsp;/g, " ");
}
...
// .wrangler/tmp/bundle-b1UH35/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-b1UH35/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7141437322715465.js.map
